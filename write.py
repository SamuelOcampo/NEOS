...
def write_to_csv(results, filename):
    ...
    with open(filename, "w") as f:
        ...
        for result in results:
            time_str, distance, velocity = itemgetter(
                "time_str", "distance", "velocity"
            )(result)
            designation, name, diameter, hazardous = itemgetter(
...
def write_to_json(results, filename):
    ...
    """
    with open(filename, "w") as f:
        json.dump(list([result.serialize() for result in results]), f)
